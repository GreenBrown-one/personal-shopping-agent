...
def test_initial_migration_upgrades_and_downgrades(tmp_path: Path) -> None:
    ...
    assert {item["referred_table"] for item in inspector.get_foreign_keys("candidate_scores")} == {
        "products",
        "shopping_requests",
        "shopping_workflows",
    }
    assert {item["referred_table"] for item in inspector.get_foreign_keys("shopping_reports")} == {
        "shopping_requests",
        "shopping_workflows",
    }
    ...
    with engine.connect() as connection:
        assert connection.scalar(text("SELECT version_num FROM alembic_version")) == "20260809_0007"
    engine.dispose()
