...
> 当前状态：M0–M6 的首轮架构与发布基线已完成。系统具备京东结构化采集适配、可恢复决策管线、确定性排名、可审计 Markdown/安全 HTML、OpenAI/DeepSeek 可选解释、私有导出与显式删除，以及安装和安全门禁。默认服务器仍不启用真实平台访问；真实页面采集须由调用方提供受安全策略约束的采集器和官方证据提供器。跨平台报价、下单和支付尚未实现。
...
完整设计约束见 [DESIGN.md](DESIGN.md)，安全与发布边界见 [SECURITY.md](SECURITY.md)，协作规则见
[AGENTS.md](AGENTS.md)。
...
文件型 SQLite 在 POSIX 上必须仅当前用户可访问。`migrate` 会以 `0600` 创建或修复普通数据库文件，
新建直接父目录使用 `0700`；符号链接、额外硬链接或非普通文件会被拒绝。`doctor` 的
`private_file_permissions` 为 `false` 时数据库不算 ready，且输出仍不包含数据库路径。Windows 上该字段
为 `null`，应把数据库放在仅当前账号可访问的 NTFS 目录中。
...
M6 的第五个切片完成本地文件与发布安全审查。数据库、浏览器资料目录、截图和 Workflow 导出均具有
明确的私有权限、普通文件和链接约束；数据库权限漂移会让 `doctor` 与默认 MCP 失败关闭，`migrate`
可以安全修复普通文件。浏览器资料或截图目录权限过宽时拒绝使用，截图失败会清理局部文件。独立
`SECURITY.md` 记录威胁模型、非目标、平台差异、事故报告方式和发布门禁，隔离 wheel 验收会验证新建
数据库的权限诊断。至此 M6 首轮发布基线完成。
