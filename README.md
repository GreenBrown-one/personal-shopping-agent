...
> 当前状态：M0–M2 已完成；M3 已具备受控浏览器、安全导航、低频访问协调、京东搜索与详情解析、结构化观察存储、请求级事务、严格领域转换和官方证据核验事务；M4 已具备确定性规格规范化、条件效用评分、报价不确定性调整成本和证据置信度基础。最终排名、真实购物平台或厂商页面采集和跨平台报价仍未启用。
...
M4 的第二个切片增加不依赖 LLM 的纯决策基础。用户条件只接受方向明确、非负且单位与规范事实一致的数值边界；每项保留满足、未满足、缺失、冲突或无法解析状态，硬性条件在排序前单独设闸。报价继续遵循既有价格层降级顺序，并用可解释的店铺、价格层、库存和地区不确定性系数形成比较成本；缺货、币种不一致及缺少条件说明的条件价不会进入最佳报价。该结果尚未加入证据置信度、数据库事务或 `candidates_scored` 状态，因此不是最终推荐。

M4 的第三个切片增加纯证据置信度评估。它按条件权重分别计算事实完整度、来源可靠性、新鲜度和一致性，保留实际参与计算的 Evidence ID 与原因码，并用四项乘积调整上一阶段效用。缺失或冲突事实不会被猜测；不同 URL 的证据只有在规范化后得到同一确定值时才算多来源一致；未来时间记录、跨请求数据和重复证据 ID 会被拒绝。来源类型和规格年龄只提供已文档化的默认先验，显式评估值可以覆盖但不会写回原 Evidence。该结果仍未持久化或推进 `candidates_scored`，也不代表最终排名。
