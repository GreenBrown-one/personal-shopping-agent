...
| 京东用户辅助采集（你保存页面，程序分析；默认） | 已支持（`list_pages_to_save` + `run_shopping_pipeline`） |
| 京东自动浏览器采集 | 实验性；首次实网验收被京东风控拦截 |
...
（实验性自动采集才需要）打开浏览器让你亲自登录京东、可选下载芯片跑分榜。默认使用用户辅助模式。完成后完全退出并重新打开 Claude Desktop。
