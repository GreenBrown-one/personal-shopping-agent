...
> 当前状态：M0–M2 已完成；M3 已具备受控浏览器、安全导航、低频访问协调、京东搜索与详情解析、结构化观察存储、请求级事务、严格领域转换和官方证据核验事务；M4 已具备确定性规格规范化、条件效用、报价不确定性调整成本、证据置信度、预算分层、Pareto 前沿和最终排名；M5 已具备不依赖 LLM 的可审计 Markdown 报告、原子持久化、受事实白名单约束的 OpenAI/DeepSeek 解释适配器、默认关闭的运行时选择、安全展示层及报告 MCP 工具。真实购物平台或厂商页面采集、跨平台报价、HTML 报告，以及平台采集和评分的 MCP 端到端装配仍未启用。
...
这些工具不会访问购物平台、下单或支付。只有最后一个解释工具在已显式配置提供方时可能访问模型 API；
其 MCP 注解标记为只读、非幂等且可能访问开放网络，普通报告读取则标记为只读、幂等且仅本地。
