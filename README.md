...
M0–M6 的首轮架构与发布基线已经完成，核心代码可安装、可测试，也适合继续由人类或 AI 维护。不过它目前是“开发者可用的本地 MCP 基线”，还不是下载后即可自动访问电商平台的普通用户产品。
...
| 京东搜索与详情解析、可恢复端到端管线 | 内部已实现，真实采集需显式注入安全采集器 |
...
当前基线包含 392 项测试，语句和分支覆盖率均为 100%。隔离验收会安装构建出的 wheel，验证版本、迁移、模板、两个命令入口、数据库权限和默认 MCP 组合。
