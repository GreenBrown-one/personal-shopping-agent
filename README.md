...
> 当前状态：M0–M2 已完成；M3 已具备受控浏览器、安全导航、京东搜索、京东详情解析器、结构化观察存储、请求级采集事务、严格领域转换和官方证据核验事务。当前仍未启用真实购物平台或厂商页面采集、跨平台报价或推荐算法。
...
当前的浏览器和京东解析器尚未注册为 MCP 工具，也没有默认启用真实平台白名单。低频策略和官方规格交叉核验完成前，`shopping_agent_status` 会继续如实报告 `platform_collection=false`。
