...
> 当前状态：M0–M2 已完成；M3 已具备受控浏览器、安全导航和首个京东搜索解析器。当前仍未启用真实购物平台采集、跨平台报价或推荐算法。
...
当前的浏览器和京东解析器尚未注册为 MCP 工具，也没有默认启用真实平台白名单。京东详情适配器、低频策略和访问边界核对完成前，`shopping_agent_status` 会继续如实报告 `platform_collection=false`。
