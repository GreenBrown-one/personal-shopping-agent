...
首次实网验收发现京东会拦截自动化浏览器的搜索（见 `docs/LIVE_JD_ACCEPTANCE.md`），因此默认方式改为
**你在自己的浏览器里看页面，程序负责分析**，不访问网络、没有封号风险：

1. 在 Claude Desktop 里说出需求，AI 会先调用 `review_shopping_request` 再创建任务；
2. AI 调用 `list_pages_to_save`，告诉你要打开的京东搜索页链接；
3. 你用自己的 Chrome/Edge 打开链接，按 `Ctrl+S`，保存类型选“网页，全部”或“网页，单个文件”，保存到
   项目的 `data\inbox` 文件夹（首次运行时自动创建）；
4. 再让 AI 调用 `list_pages_to_save`，它会列出需要打开的商品页（默认前 3 个），逐个打开并保存；
5. AI 调用 `run_shopping_pipeline`：缺哪个页面会以 `page_not_saved` 告诉你确切链接，保存后再运行即可
   从上次进度继续，直到生成报告。

这些页面可能包含你的京东昵称或收货地区，只保存在本机、不进入 Git。京东页面改版后若解析失败，请运行
`improvement-case` 并把脱敏结果与页面结构问题反馈给维护者。
