...
代码中的 `evolution/` 包（自动进化层）实现了这张表：`personal-shopping-agent improvement-case
<WORKFLOW_UUID>` 在本地打印案例预览。`sanitized_input` 以“需求结构摘要”实现（条件数量、受支持规范键、
是否设置地区/弹性预算、币种），不含用户原话；期望/实际表现由阶段和错误码推导，模型校验器会拒绝任何
被改写的文本。它不写文件、不联网、不注册 MCP 工具，其他业务层也不能导入它。
...
2. **自然语言契约**：为常见购物需求增加宿主提示与结构化追问样例，但继续由类型模型兜底。需求层已提供确定性审查与追问建议。
3. **真实京东组合**：受控浏览器生命周期和精确主机策略已提供显式入口；人工登录门与真实环境验收仍须在用户电脑完成。
4. **本地反馈与回放**：先让用户预览脱敏案例，再选择是否创建私有 Issue。本地预览已提供；脱敏 fixture 回放仍待实现。
